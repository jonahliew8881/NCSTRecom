...
const R_COLOR = { 5:'#137035', 4:'#2A4A6B', 3:'#5A7A98', 2:'#8AAAC8', 1:'#D97706' };
    ...
    Papa.parse(CSV_URL, {
        ...
        complete: function(results) {
            ...
            rows.forEach((row, index) => {
                ...
                // 1. 讀取基礎客觀數據
                const rawGF = String(row['無穀物'] || '');
                const isGF = (rawGF.includes('1') || rawGF.includes('無') || rawGF.includes('是') || rawGF.includes('Y') || rawGF.includes('✓')) ? 1 : 0;
                
                const rawBad = String(row['不推薦'] || '');
                const isBad = (rawBad.includes('1') || rawBad.includes('紅') || rawBad.includes('不') || rawBad.includes('是') || rawBad.includes('Y') || rawBad.includes('✓')) ? 1 : 0;

                const type = row['種類'] ? row['種類'].trim() : '乾糧';
                const protein = parseFloat(row['蛋白質']) || null;
                const carb = parseFloat(row['碳水']) || null;
                const fat = parseFloat(row['脂肪']) || null;
                const phos = parseFloat(row['磷']) || null;
                const ash = parseFloat(row['灰質']) || null;
                const fib = parseFloat(row['纖維']) || null;

                // 🧠 2. 升級版 AI 營養師自動計分系統 (Advanced Auto Scoring)
                let score = 3.0; // 起步分 3 分

                if (isBad) {
                    score = 1; // 致命地雷，直接 1 分
                } else {
                    // 【種類加成】：貓咪極需要水分，罐頭/生肉糧天生具備補水優勢
                    if (type.includes('罐') || type.includes('濕') || type.includes('生肉')) {
                        score += 0.5;
                    }

                    // 【蛋白質評分】：貓是絕對肉食動物
                    if (protein !== null) {
                        if (protein >= 50) score += 1.0;
                        else if (protein >= 40) score += 0.5;
                        else if (protein < 25) score -= 1.5;
                        else if (protein < 30) score -= 0.5;
                    }
                    
                    // 【碳水評分】：越低越好，無碳水最佳
                    if (carb !== null) {
                        if (carb <= 5) score += 1.0;
                        else if (carb <= 10) score += 0.5;
                        else if (carb > 25) score -= 1.5;
                        else if (carb > 20) score -= 1.0;
                        else if (carb > 15) score -= 0.5;
                    }

                    // 【脂肪評分】：適中為佳
                    if (fat !== null) {
                        if (fat >= 15 && fat <= 22) score += 0.5;
                        else if (fat < 10) score -= 0.5;
                    }

                    // 【扣分項目】：過高的灰質、纖維、磷代表品質參差或加重器官負擔
                    if (ash !== null && ash > 10) score -= 0.5;
                    if (fib !== null && fib > 5) score -= 0.5;
                    if (phos !== null && phos > 1.5) score -= 0.5;
                    ...
                    if (score > 5) score = 5; // 封頂 5 分
                    if (score < 1) score = 1; // 墊底 1 分
                }
...
function doRecommend() {
  const needBtn = document.querySelector('#needChips .chip.on');
  const ageBtn = document.querySelector('#ageChips .chip.on');
  const typeBtn = document.querySelector('#typeChips .chip.on');
  const meatBtn = document.querySelector('#meatChips .chip.on');

  const need = needBtn ? needBtn.textContent.replace(/[^\u4e00-\u9fa5]/g, '') : '';
  const age = ageBtn ? ageBtn.textContent.replace(/[^\u4e00-\u9fa5]/g, '') : '';
  const type = typeBtn ? typeBtn.textContent.replace(/[^\u4e00-\u9fa5]/g, '') : '全部';
  const meat = meatBtn ? meatBtn.getAttribute('data-val') : '';

  let res = DB.filter(f => f[9] !== 1); // 預設踢走不推薦名單

  if (type !== '全部') {
    res = res.filter(f => f[2] && f[2].includes(type));
  }

  // 🥩 聰明版肉類過濾：避開「牛磺酸」同「火雞」嘅字眼重疊
  if (meat) {
      const keywords = meat.split('|');
      res = res.filter(f => {
          const detail = DETAIL[f[0]] || {};
          let searchString = (f[1] + " " + (f[3]||"") + " " + (detail.n||"") + " " + (detail.i||"")).toLowerCase();
          // ✨ 神奇魔法：將字眼偷偷換走，避免誤中副車
          // ✨ 終極防誤判魔法：將容易撞名嘅非肉類/其他動物字眼換走
          searchString = searchString
              .replace(/牛磺酸/g, 'taurine')
              .replace(/火雞/g, 'turkey')
              .replace(/牛奶/g, 'milk')
              .replace(/牛油果/g, 'avocado')
              .replace(/羊奶/g, 'goatmilk')
              .replace(/雞蛋/g, 'egg')
              .replace(/鱷魚/g, 'crocodile')
              .replace(/鮑魚/g, 'abalone');
          return keywords.some(kw => searchString.includes(kw));
      });
  }

  if (age === '幼貓') {
      res = res.filter(f => f[4] !== null && f[4] >= 40);
  } else if (age === '老貓') {
      res = res.filter(f => f[11] !== null ? f[11] <= 1.0 : f[4] !== null && f[4] <= 35);
  }

  // 🏥 聰明版腎病過濾：如果有磷數據就睇磷 (<=1.0)，如果冇磷數據，就睇蛋白質是否適中 (<=35%)
  if (need === '腎病護理') {
      res = res.filter(f => {
          if (f[11] !== null) return f[11] <= 1.0;
          return f[4] !== null && f[4] <= 35; 
      });
  } else if (need === '泌尿問題') {
      if (type === '全部') {
          res = res.filter(f => (f[2] && f[2].includes('罐頭')) || (f[2] && f[2].includes('濕')));
      }
  } else if (need === '減重') {
      res = res.filter(f => (f[5] !== null && f[5] <= 15) || (f[6] !== null && f[6] <= 12));
  } else if (need === '腸胃敏感') {
      res = res.filter(f => f[7] === 1);
  }
      ...
      <div class="card ${cls}">
        <div class="card-top" onclick="openDetail(${f[0]})" style="cursor:pointer">
          ...
          <div class="card-info">
            ...
            <div class="paws" style="display: flex; align-items: center;">
             <span style="font-weight:bold; font-size:1.1rem; margin-right:6px; color:${rC};">${f[8]} 分</span>
             ${paws} 
             <span class="rating-lbl" style="color:${rC}; margin-left:6px;">${R_LABEL[f[8]]}</span>
            </div>
          </div>
        </div>
...
// ✨ 終極防誤判魔法：將容易撞名嘅非肉類/其他動物字眼換走
          searchString = searchString
              ...
              .replace(/雞蛋/g, 'egg')
              .replace(/鱷魚/g, 'crocodile')
              .replace(/鮑魚/g, 'abalone');          return keywords.some(kw => searchString.includes(kw));
      });
  }
