...
function openDetail(oi) {
  ...
  let ppRatioHtml = '';
  if (f[11] && f[4]) {
      const ppRatio = (f[11] / f[4]).toFixed(4);
      const isOk = ppRatio < 0.025;
      ppRatioHtml = `
        <div class="pp-ratio-box ${isOk ? 'ok' : 'warn'}">
            <strong>磷/蛋白比值 (P:P Ratio): ${ppRatio}</strong><br>
            ${isOk ? '對腎臟負擔較小 (< 0.025)' : '數值偏高，腎貓需留意 (> 0.025)'}
        </div>
      `;
  }
...
function openKnow(id) {
  ...
  document.getElementById('kBody').innerHTML = `<div style="font-size:.82rem;line-height:1.7;color:var(--text)">${k.a}</div>`;
